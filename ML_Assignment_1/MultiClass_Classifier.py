...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 22 11:50:32 2022
...
def getPerceptronModelWeights(max_itr, dataset = training_data):
    ...
    weights = np.zeros((10,data_width))
    
    for itr in range(max_itr):
        for data_point in dataset:
            
            label_values = []
            data_label = int(data_point[0])
            
            x_values = data_point[1:]
            
            for label in range(10):
            
                f_x = np.sum(np.multiply(x_values,weights[label]));
                
                label_values.append(f_x);
            
            
            # Label found for the given training data point.
            found_label = np.argmax(label_values)
            
            if(found_label != data_label):
                weights[found_label] = weights[found_label] - x_values
                weights[data_label] = weights[data_label] + x_values
...
def checkPlainPerceptronModel(calculated_weights, dataset):
    ...
    for data_point in dataset:
        ...
        for label in range(10):
            ...
            f_x = np.sum(np.multiply(x_values,calculated_weights[label]));
...
def getAveragedPerceptronModelWeights(max_itr,dataset = training_data):
    ...
    stored_weights = {}
    stored_weight_counts = {} 
    
    initial_label_weights = np.zeros(data_width)

    
    for label in range(10):
        stored_weights[label] = [initial_label_weights]
        stored_weight_counts[label] = [1]
    ...
    for itr in range(max_itr):
        ...
        for data_point in dataset:
            ...
            for label in range(10):
                label_weight = stored_weights[label][-1]
                
                f_x = np.sum(np.multiply(x_values,label_weight));
            ...
            if(data_label != found_label):
                ...
                found_label_weight = stored_weights[found_label][-1]
                data_label_weight = stored_weights[data_label][-1]
                
                stored_weights[found_label].append(found_label_weight - x_values)
                stored_weights[data_label].append(data_label_weight + x_values)
                
                
                stored_weight_counts[found_label].append(1)
                stored_weight_counts[data_label].append(1)
                
            else:
                
                stored_weight_counts[data_label][-1]+=1;
...
def checkAveragedPerceptronModel(weight_counts, stored_weights, dataset):
    ...
    for label in range(10):
        ...
        weight_counts[label] = np.array(weight_counts[label]).reshape(len(weight_counts[label]),1).tolist()
        label_weight = np.sum(np.multiply(weight_counts[label],stored_weights[label]),axis=0)
    ...
    for data_point in dataset:
        ...
        for label in range(10):
            f_x = np.sum(np.multiply(x_values,label_weights[label]))
            
            fx_values.append(f_x)
        
        
        found_label = np.argmax(fx_values)
...
def getPassiveAggressiveModelWeights(max_itr,dataset = training_data):
    ...
    for itr in range(max_itr):
        for data_point in dataset:
            ...
            if(np.absolute(np.sum(np.multiply((weights[data_label] - weights[found_label]),x_values))) < 1):
                
                # In the denominator multiplying by root 2 because the norm of difference of F(x,y) and F(x,y') is equal to root to times norm of x.
                learning_rate = (1 - np.sum(np.multiply((weights[data_label] - weights[found_label]),x_values)))/(np.sqrt(2) * (np.linalg.norm(x_values)**2))
                
                weights[found_label] = weights[found_label] - (learning_rate * x_values)
                weights[data_label] = weights[data_label] + (learning_rate * x_values)
